...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " byiness-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  byiness-tx [options] <hex-tx> [commands]  Update hex-encoded byiness transaction\n" +
            "or:     byiness-tx [options] -create [commands]   Create hex-encoded byiness transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded byiness transaction
