...
const char * const BITCOIN_CONF_FILENAME = "byiness.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "byinessd -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //byiness-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Byiness Coin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Byiness Coin
    // Mac: ~/Library/Application Support/Byiness Coin
    // Unix: ~/.byiness
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Byiness Coin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Byiness Coin";
#else
    // Unix
    return pathRet / ".byiness";
#endif
#endif
}
