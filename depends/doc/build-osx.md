...
1. Clone the Byiness Coin - Digital Revelation source code:
    ```shell
    git clone https://github.com/Digital-Revelation/Byiness
    cd byiness
    ```

2.  Build Byiness Coin:

    Configure and build the headless Byiness Coin binaries as well as the GUI (if Qt is found).
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, Byiness Coin may be
...
## Running
Byiness Coin is now available at `./src/byinessd`

Before running, you may create an empty configuration file:
```shell
mkdir -p "/Users/${USER}/Library/Application Support/Byiness Coin"

touch "/Users/${USER}/Library/Application Support/Byiness Coin/byiness.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Byiness Coin/byiness.conf"
```

The first time you run byinessd, it will start downloading the blockchain. This process could
take many hours, or even days on slower than average systems.

You can monitor the download process by looking at the debug.log file:
```shell
tail -f $HOME/Library/Application\ Support/Byiness Coin/debug.log
```

## Other commands:
```shell
./src/byinessd -daemon      # Starts the byiness daemon.
./src/byiness-cli --help    # Outputs a list of command-line options.
./src/byiness-cli help      # Outputs a list of RPC commands when the daemon is running.
```
...
- The Apple keyholder uses this unsigned app to create a detached signature, using the
  script that is also included there. Detached signatures are available from this [repository](https://github.com/Digital-Revelation/Byiness-detached-sigs).
