...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci, use_term_control):
    ...
    # Warn if byinessd is already running
    # pidof might fail or return an empty string if byinessd is not running
    try:
        if subprocess.check_output(["pidof", "byinessd"]) not in [b'']:
            print("%sWARNING!%s There is already a byinessd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
...
class RPCCoverage():
    ...
    commands per `byiness-cli help` (`rpc_interface.txt`).
